...
# habitat also uses egl, so it's a good place to look for egl errors
# https://github.com/facebookresearch/habitat-sim/issues/664
...
parser.add_argument('--dataset_type', default='imagefolder', type=str, choices=['imagefolder', 'shader_list_file'], help='path to dataset')
...
# parameters for shader_list_file live generation
parser.add_argument('--generation-resolution-multiplier', default=1, type=float, help='How extra big should the images be when generating them.')
...
parser.add_argument('--transform-before-mixing', type=str2bool, default='False', help='Whether to apply a transformation before the mixing')
...
def main():
    ...
    print("Running with {} gpus per node".format(ngpus_per_node))
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset_type == 'shader_list_file':
    ...
    if args.dataset_type == 'imagefolder':
      ...
      shader_list_file = args.data[0]
      shader_files = read_text_file_lines(shader_list_file)

      parameter_diversity = shader_list_file.endswith('_with_parameter_diversity')
      ...
      if len(args.rendering_gpus) == 1 and args.rendering_gpus[0] == -1:
        if 'CUDA_VISIBLE_DEVICES' in os.environ.keys():
          ...
          rendering_gpus = [int(os.environ['CUDA_VISIBLE_DEVICES'].split(',')[gpu])]
        else:
          rendering_gpus = [gpu]
      else:
        rendering_gpus = args.rendering_gpus
      ...
      train_dataset = ModernGLOnlineDataset(shader_files,
                                         transform,
      ...
      # "forkserver" instead of fork either here or on top as mp.set_start_method("forkserver", force=True) causes the program to fail
